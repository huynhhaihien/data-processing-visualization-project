...
# with open("clean_data.csv", encoding="utf-8", mode="w", newline='') as file_csv:
# 	header = ["sbd", "tên", "dd", "mm", "yy", "toán", "ngữ văn", "khxh", "khtn", "lịch sử", "địa lý", "gdcd", "sinh học", "vật lý", "hóa học", "tiếng anh"]
...
print(scores[1])
	...
	# for subject in ["toán", "ngữ văn", "khxh", "khtn", "lịch sử", "địa lý", "gdcd", "sinh học", "vật lý", "hóa học", "tiếng anh"]:
	# 	if subject in scores_list:
	# 		data.append(scores_list[scores_list.index(subject)+1])
	# 	else:
	# 		data.append("-1")

	# with open("clean_data.csv", mode="a", encoding='utf-8', newline='') as file_csv:
	# 	writer = csv.writer(file_csv)
